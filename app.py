...
for idx, img in enumerate(IMAGES):
    with cols[idx % 3]:
        ...
        # Commentaires
        st.write("### Commenter")
        with st.form(f"form-{img['id']}"):
            name = st.text_input("Votre nom", key=f"name_{img['id']}")
            text = st.text_area("Commentaire", key=f"text_{img['id']}")
            ...
            if submit:
                if not name or not text:
                    ...
                    st.session_state.comments[img["id"]] = comments
                    st.success("Commentaire envoyé ✅")
        # Afficher les commentaires
        if st.session_state.comments.get(img["id"]):
            st.markdown("#### Commentaires publics")
            for c in st.session_state.comments[img["id"]]:
                st.markdown(
                    f"""
                    <div style="border:1px solid #eee;border-radius:10px;padding:.6rem .7rem;margin:.5rem 0;background:#fff">
                    <strong>{c['name']}</strong> · <span style='color:#888'>{c['ts']}</span>
            ...
            st.markdown("<em style='color:#bbb'>Aucun commentaire pour ce motif.</em>", unsafe_allow_html=True)
